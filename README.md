# Anna Pastushko
### AWS Solutions Architect | Data & AI | [LinkedIn](https://linkedin.com/in/annpastushko)
...
Focus areas:
- AI/ML solutions and MLOps automation
- Data platform architecture (lakes, warehouses, event-driven pipelines)
- Cloud migration and application modernization
- Cost optimization and Well-Architected reviews
- POC Development using Python and Go
...
Processing Architecture: Amazon OpenSearch serves as the unified search layer, handling both dense vector and sparse keyword retrieval in a single database. Bedrock Knowledge Bases was ruled out due to insufficient control over chunking and parsing of complex policy tables. A dedicated vector database like Milvus was ruled out due to higher operational overhead and less mature sparse search compared to OpenSearch, which provides production-proven hybrid search in a single, unified system. All embeddings, reranking, and LLM inference run on Amazon Bedrock, ensuring PHI and PII never leave the AWS network boundary. Amazon Bedrock Prompt Management is integrated alongside Git — Git remains the single source of truth, and the CI/CD pipeline pushes validated prompt updates directly to Bedrock, allowing production containers to consume them without container rebuilds.
